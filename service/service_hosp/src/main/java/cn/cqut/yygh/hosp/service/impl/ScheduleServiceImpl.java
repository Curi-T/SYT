...
import cn.cqut.yygh.hosp.mapper.ScheduleMapper;
...
public class ScheduleServiceImpl extends ServiceImpl<ScheduleMapper, Schedule> implements ScheduleService {
    ...
    public Object getBookingScheduleRule(Integer page, Integer limit, String hoscode, String depcode) {
        ...
        Aggregation agg = Aggregation.newAggregation(Aggregation.match(criteria), Aggregation.group("workDate")//分组字段
                .first("workDate").as("workDate").count().as("docCount").sum("availableNumber").as("availableNumber").sum("reservedNumber").as("reservedNumber"));
    ...
    private IPage<Date> getListDate(int page, int limit, BookingRule bookingRule) {
        ...
        Date date = new Date();
        try {
            //TODO 153 修改系统时间，使得时间符合数据库中时间
            date = new SimpleDateFormat("yyy-mm-dd").parse("2021-1-1");
        } catch (Exception e) {
            e.printStackTrace();
        }
        DateTime releaseTime = this.getDateTime(date, bookingRule.getReleaseTime());
        //预约周期
        ...
        for (int i = 0; i < cycle; i++) {
            //计算当前预约日期
            DateTime curDateTime = new DateTime(date).plusDays(i);
